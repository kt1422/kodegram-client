...
const ModalChat = (props) => {
    ...
    const handleChat = (userData) =>{
        props.handleNewChat(userData);
    }

    return (
        <div>
            <div className="modal fade" id={`modalChat`} tabIndex="-1" aria-labelledby="modalChatLabel" aria-hidden="true" style={{color: "black"}}>
                <div className="modal-dialog modal-dialog-centered modal-dialog-scrollable">
                    <div className={`modal-content ${props.theme}`}>
                        <div className="modal-header d-flex" data-bs-theme={`${props.theme}`}>
                            <h1 className="modal-title fs-5 flex-fill text-center">New Message</h1>
                            <button type="button" className="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div className="modal-body" style={{height: 500}}>
                            <div className="input-group mb-3">
                                <span className="input-group-text bg-white border-info border-end-0">
                                    <FontAwesomeIcon icon="fa fa-search" style={{color: "#aaa"}} />
                                </span>
                                <input name='search' type="text" className="form-control shadow-none ps-0 border-info border-start-0" placeholder="Search name or username" aria-label="Username" aria-describedby="basic-addon1" style={{textAlign: "left"}} onChange={(e)=>handleSearch(e)}/>
                            </div>
                            {
                            (users.length>0 && users[0]!="loading")?
                                ...
                                <div key={data.user_id} className='d-flex align-items-start mb-3'>
                                    ...
                                    <div className='flex-fill'>
                                        <button className="btn btn-primary my-1 float-end" onClick={()=>handleChat(data)} data-bs-dismiss="modal">Message</button>
                                    </div>
                                </div>
                            ))
                            :
